...
func TestPluginPathsAreUnaltered(t *testing.T) {
	...
	ioStreams, _, _, errOut := fabricator.NewTestIOStreams()
	...
	if _, err := ioutil.TempFile(tempDir, "fabricator-"); err != nil {
		t.Fatalf("unexpected error %v", err)
	}
	if _, err := ioutil.TempFile(tempDir2, "fabricator-"); err != nil {
		t.Fatalf("unexpected error %v", err)
	}
...
func TestPluginPathsAreValid(t *testing.T) {
	...
	tc := []struct {
		...
		{
			name:        "ensure no plugins found if no files begin with fabricator- prefix",
			pluginPaths: []string{tempDir},
			verifier:    newFakePluginPathVerifier(),
			pluginFile: func() (*os.File, error) {
				return ioutil.TempFile(tempDir, "notfabricator-")
			},
			expectErr: "error: unable to find any fabricator plugins in your PATH\n",
		},
		{
			...
			verifier:    newFakePluginPathVerifier(),
			pluginFile: func() (*os.File, error) {
				return ioutil.TempFile(tempDir, "fabricator-")
			},
	...
	for _, test := range tc {
		t.Run(test.name, func(t *testing.T) {
			ioStreams, _, out, errOut := fabricator.NewTestIOStreams()
			o := &Options{
